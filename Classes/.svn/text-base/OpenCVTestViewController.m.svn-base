...
- (void)opencvAdaThreshold {
	if(imageView.image) {
		...
		// Detect edge
		IplImage *img2 = cvCreateImage(cvGetSize(img), IPL_DEPTH_8U, 1);
		cvAdaptiveThreshold(img, img2, 255, 0 , 0, 71, 40.0 );
		cvReleaseImage(&img);
...
- (void) opencvFaceDetect:(UIImage *)overlayImage  {
	if(imageView.image) {
		...
		CvSeq* faces = cvHaarDetectObjects(small_image, cascade, storage, 1.2f, 2, CV_HAAR_DO_CANNY_PRUNING, cvSize(20, 20), cvSize(20, 20)); //
...
- (IBAction)adaThreshold:(id)sender {
	[self showProgressIndicator:@"Processing"];
	[self performSelectorInBackground:@selector(opencvAdaThreshold) withObject:nil];
}
